...
export interface MemoryAtlasCanvasProps {
...
export function MemoryAtlasCanvas({
  ...
  const [focus, setFocus] = useState<AtlasFocus>({ kind: 'root' })
  ...
  const atlasProjections = (): AtlasProjection[] => {
    ...
    const projectionNames = new Set<string>(projections.map((projection) => projection.name))
    for (const name of rootsByProjection.keys()) projectionNames.add(name)

    const rows = [...projectionNames].map((name) => {
      const projection = projections.find((candidate) => candidate.name === name) ?? null
  ...
  const drawFocusBar = (
    ...
    hitRegionsRef.current.push({ kind: 'control', rect: allRect, action: 'root', navDepth: 0 })
    hitRegionsRef.current.push({ kind: 'control', rect: backRect, action: 'parent', navDepth: 0 })
  ...
  const drawAddressedNode = (
    ...
    hitRegionsRef.current.push({ kind: 'addressed', rect: outer, node, navDepth })
  ...
  const drawAtlasChild = (
    ...
    if (child.kind === 'state') {
      hitRegionsRef.current.push({ kind: 'state', rect, projectionName: child.projectionName, navDepth })
  ...
  const drawProjection = (
    ...
    hitRegionsRef.current.push({ kind: 'projection', rect: outer, projectionName: atlas.name, navDepth })
  ...
  const regionAt = (x: number, y: number): HitRegion | null => {
    const contained = hitRegionsRef.current.filter((region) => contains(region.rect, x, y))
  ...
  const projectionTooltip = (projectionName: string, x: number, y: number): Tooltip => {
    const projection = projections.find((candidate) => candidate.name === projectionName)
    return {
      x,
      y,
  ...
  const handleMove = (event: ReactMouseEvent<HTMLCanvasElement>) => {
    ...
    const region = regionAt(offsetX, offsetY)
    if (!region) {
      setTooltip(null)
      setHoverTarget(null)
    ...
    setHoverTarget(regionTarget(region))
    if (region.kind === 'control') {
      setTooltip(null)
      return
    }
    setTooltip(region.kind === 'addressed'
      ? addressedTooltip(region.node, offsetX + 14, offsetY + 14)
      : projectionTooltip(region.projectionName, offsetX + 14, offsetY + 14))
  }
  ...
  const handleClick = (event: ReactMouseEvent<HTMLCanvasElement>) => {
    ...
    if (region.kind === 'control') {
      setFocus(region.action === 'root' ? { kind: 'root' } : focusParent())
      return
    }

    if (region.kind === 'addressed') {
      setFocus({ kind: 'addressed', path: region.node.path })
      onSelectNode(region.node)
      return
    }

    if (region.kind === 'projection') {
      setFocus({ kind: 'projection', projectionName: region.projectionName })
    }
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      setViewportHeight(Math.max(1, Math.floor(entry.contentRect.height)))
    })
  ...
  useEffect(() => {
    drawAtlas()
  }, [projections, addressedAtlas, activeClientId, focus, hoverTarget, viewportWidth, viewportHeight])

  return (
    <div className="memory-canvas-wrap" ref={wrapperRef}>
      <canvas
      ...
      />
      {tooltip && (
        <div className="atlas-tooltip" style={{ left: `${tooltip.x}px`, top: `${tooltip.y}px` }}>
