...
export const createServerServices = (options: ServerServicesOptions = {}): ServerServices => {
  ...
  const discovered = new ManagedDiscoveredModels({
    ...
    ensureInstalledModelInventory: inventory.ensureInstalledModelInventory?.bind(inventory),
    ensureModelInventory: inventory.ensureModelInventory?.bind(inventory),
  })
  const installations =
