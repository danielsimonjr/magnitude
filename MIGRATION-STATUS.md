...
| 3. Engine | In progress | `packages/icn-engine` scheduler, sequence pool, KV reuse, sampling config, reasoning resolution; native sampler-chain + chat-template apply wired through `@magnitudedev/icn-native`; speculative pure validation + typed native-missing errors; expanded multimodal envelope validation; `packages/icn-parity` greedy determinism harness |
...
| `@magnitudedev/icn-engine` | 40 (+3 GGUF-gated) |
| `@magnitudedev/icn-hardware` | 19 |
| `@magnitudedev/icn-server` | 20 |
| `@magnitudedev/icn-parity` | 1 (+1 GGUF-gated) |
...
- Connect engine sampling/chat-template paths to `@magnitudedev/icn-native` and add CPU token-for-token parity harness (sampler chain + `llama_chat_apply_template` + `packages/icn-parity` greedy determinism harness landed; full CommonChatTemplates Jinja inspection and grammar/reasoning-budget samplers remain).
