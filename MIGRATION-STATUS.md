...
| 4. HTTP and lifecycle | In progress | `packages/icn-server` (bootstrap, Bun HTTP, auth, fake completions, catalog/discovery/installations/instances routes via `icn-models`, worker IPC with `--fake`/`--local-engine`, memory supervisor, CLI) + `packages/icn-hardware` memory policy, CPU-side capacity summary, memory-domain snapshot building, calibration record validation, and filesystem/platform GPU probes (22 CPU tests); inference/chat/responses/HF/install routes still 501; ACN lifecycle integration pending |
...
- Metal, CUDA, Vulkan backend loading from Bun; full cuInit/Vulkan instance probes beyond filesystem loader discovery.
...
- HF catalog refresh/release materialization landed with injectable HTTP; full GGUF-header planner stub acquisition for live catalog generation still needs native/preview materialization depth.
